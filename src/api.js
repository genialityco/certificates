...
export const fetchFilteredGlobal = async (colection, filters) => {
  try {
    const response = await axiosInstanceNew.get(`/search/${colection}`, {
      params: filters,
    });
...
export const fetchAllsEvents = async () => {
  const response = await axiosInstanceNew.get('/events');
  return response.data;
};

export const fetchEventById = async (eventId) => {
  const response = await axiosInstanceNew.get(`/events/${eventId}`);
  return response.data;
};

export const createEvent = async (eventData) => {
  const response = await axiosInstanceNew.post('/events', eventData);
  return response.data;
};

export const deleteEvent = async (eventId) => {
  const response = await axiosInstanceNew.delete(`/events/${eventId}`);
  return response.data;
};

// API para certificados
export const createCertificate = async (eventId, elements) => {
  const response = await axiosInstanceNew.post(`/certificates`, { elements, eventId });
  return response.data;
};

export const updateCertificate = async (certificateId, elements) => {
  const response = await axiosInstanceNew.put(`/certificates/${certificateId}`, { elements });
  return response.data;
};

// API para asistentes
export const addAttendee = async (attendeeData) => {
  const response = await axiosInstanceNew.post(`/attendees`, attendeeData);
  return response.data;
};

export const updateAttendee = async (attendeeId, attendeeData) => {
  const response = await axiosInstanceNew.put(`/attendees/${attendeeId}`, attendeeData);
  return response.data;
};

export const deleteAttendee = async (attendeeId) => {
  const response = await axiosInstanceNew.delete(`/attendees/${attendeeId}`);
  return response.data;
};
